    ...
    <div>
      <Grid relaxed="very" columns={3}>
        {filteredSneakers.map(sneaker=>{
          return <SneakerCard status={props.status} handleClick={props.buyNowClick} buyNowClick={props.buyNowClick} sneaker={sneaker} key={sneaker.id} shoppingCartClick={props.shoppingCartClick} />
        })}
      </Grid>
    </div>
  )
